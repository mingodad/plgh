//From: tree-sitter/cli/generate/dsl.js
...
//qjs tree-sitter-generate.js ../tree-sitter/tree-sitter-gleam/grammar.js

if(scriptArgs.length > 1) {
	let fname = scriptArgs[1];
	//print(fname);
	let grammarjs = std.loadFile(fname);
	grammarjs = grammarjs.replace(/module\.exports\s*=\s*grammar\s*\(/ , "grammar(");
	//print(grammarjs);
	//std.evalScript(grammarjs);
	let grammar_json = eval(grammarjs);
	print(JSON.stringify(grammar_json.grammar, null, 2));
}
else {
	std.printf("usage: %s grammar_js_file_name\n", scriptArgs[0]);
}
